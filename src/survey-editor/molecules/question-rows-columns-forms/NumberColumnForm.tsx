...
import type { BaseRowColumnFormProps, NumberColumnFormData } from './types';
import type { BaseRowColumnOptions } from '../../../survey-types';

export function NumberColumnForm({ item, onUpdate }: BaseRowColumnFormProps) {
    const { control, handleSubmit, watch, formState: { isDirty } } = useForm<NumberColumnFormData>({
        defaultValues: {
            code: item.code,
            label: item.label,
            options: {
    ...
    const onSubmit = (data: NumberColumnFormData) => {
        const updatedItem = {
            ...item,
            code: data.code,
            label: data.label,
