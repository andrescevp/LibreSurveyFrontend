...
import type { BaseRowColumnFormProps, NumberRowFormData } from './types';
import type { BaseRowColumnOptions } from '../../../survey-types';

export function NumberRowForm({ item, onUpdate }: BaseRowColumnFormProps) {
    const { control, handleSubmit, watch, formState: { isDirty } } = useForm<NumberRowFormData>({
        defaultValues: {
            code: item.code,
            label: item.label,
            options: {
    ...
    const onSubmit = (data: NumberRowFormData) => {
        const updatedItem = {
            ...item,
            code: data.code,
            label: data.label,
